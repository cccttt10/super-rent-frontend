const validateVehicle = values => {
    ...
    if (!values.status) errors.status = ['You must set vehicle status'];

    if (!values.make) errors.make = ['You must set vehicle make'];

    if (!values.model) errors.model = ['You must set vehicle model'];

    if (!values.year) errors.year = ['You must set vehicle year'];

    if (!values.color) errors.color = ['You must set vehicle color'];

    if (!values.vehicleTypeName)
        errors.vehicleTypeName = ['You must specify a vehicle type'];

    if (!values.location) errors.location = ['You must set vehicle location'];
